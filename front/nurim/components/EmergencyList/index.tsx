...
const EmergencyList: React.FunctionComponent<DialogComponentProps> = () => {
  ...
  let buttonEvent = () => {setLogin(true)};

  //로그인시에 이하를 적용
  if (login == true) {
    //버튼의 기능을 로그아웃으로 변경
    buttonEvent = () => {setLogin(false)}
    //버튼 타이틀 변경
    loginButton = '로그아웃하기'
        ...
        />
          ...
          >
            ...
            <ListItem
              ...
              onPress={()=>{Linking.openURL(`tel:${l.phoneNumber}`)}}//리스트의 phoneNumber 연락
            >
          <Avatar source={require('../../assets/images/PHONE_CALL.png')} />
            <ListItem.Content>
              <ListItem.Title style={{ color: 'red', fontSize: 25, fontWeight: 'bold' }}>
                {l.name}
              </ListItem.Title>
